...
MCP 服务器在 AgentLoop 启动时通过配置建立连接，发现其 tools/resources/prompts 后包装成 `MCPToolWrapper` 注册到共享的 `ToolRegistry`。之后 LLM 调用 MCP 工具和调用普通工具走的是同一条路径，只是 `execute` 内部通过 MCP session 转发给远程服务器执行。
